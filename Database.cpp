...
class Database {
    struct Data{
        ...
        int age;
        std::string phoneNo;
        Data(std::string name,std::string email,int id,int age, std::string phoneNo)  : name(name), email(email), id(id), age(age), phoneNo(phoneNo) {};
    };
...
public:
    void printData() {
        for(auto data : datas) {
            std::cout << " ID: " << data.id << " Name : " << data.name << " Email: " << data.email << " Age: " << data.age << " Phone No: " << data.phoneNo << "\n";
        }
    }
    void printJsonData() {
        for(auto data : datas) {
    ...
    void generateData(int numberOfData) {
        std::map<std::string, int> history;
        //Always have or Pre-Data;
        std::vector<std::string> names = generateNames();
        std::string emails[] = {"gmail", "hotmail", "ymail"};
        std::string phoneOperators[] = {"+88016", "+88017", "+88013", "+88019"};
        ...
        for(int i = 0; i < numberOfData; ++i) {
            ...
            //generating phone number extra digits
            long long phone = rand()*rand()*rand()*rand()*rand()*rand()*rand();
            if(phone < 0) phone *= -1;
            ...
            try{
                if(history.at(name)) {
                    datas.emplace_back(name, name + "@" + emails[rand() % 3] + ".com", i, rand()%31 + 15, phoneOperators[rand() % 4] + std::to_string(phone));
                    history.at(name)++;
                }
            }
            catch(std::exception e) {
                history.insert(std::pair<std::string,int>(name,1));
                datas.emplace_back(name, name + "@" + emails[rand() % 3] + ".com", i, rand()%31 + 15, phoneOperators[rand() % 4] + std::to_string(phone));
            }
        }
    }
